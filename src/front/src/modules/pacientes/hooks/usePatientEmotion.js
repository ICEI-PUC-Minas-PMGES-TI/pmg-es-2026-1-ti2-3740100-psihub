...
export function usePatientEmotion(onToast) {
    ...
    useEffect(() => {
        ...
        patientApi.listEmotionRecords(controller.signal)
            .then((data) => {
                setRecords(data || []);
                setError('');
            })
    ...
    async function handleSubmit(event) {
        ...
        try {
            if (editingId) {
                await patientApi.updateEmotionRecord(editingId, payload);
            } else {
                await patientApi.createEmotionRecord(payload);
            }
            setRecords(await patientApi.listEmotionRecords());
            setForm(initialForm);
            setEditingId(null);
